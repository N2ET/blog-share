 ...
 * @author Ican Bachors
 * @carrier CLZ 19/1/16
 * https://blog.csdn.net/changanyi/article/details/86523322
 */
class ImgCompressor {
    ...
    public function compress(  $level = 0) {
        ...
        // Uniform variable syntax is available in PHP 7 only
        // $image = ( 'imagecreatefrom' . basename($type) )($this->imagePath);
        $imageFn = 'imagecreatefrom' . basename($type);
        $image = $imageFn($this->imagePath);
...
//            return $this;
    ...
    function resize($width, $height) {
        ...
        $imageFn = $imageSrc = 'imagecreatefrom' . basename($this->imageInfo['mime']);
        $imageSrc = $imageFn($this->imagePath);
...
//        ( 'image' . basename($this->imageInfo['mime']) )($image, $this->imageCompressPath);
        $imageFn = 'image' . basename($this->imageInfo['mime']);
        $imageFn($image, $this->imageCompressPath);
